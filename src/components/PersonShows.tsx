...
export const PersonShows: FC<Props> = ({ credits }) => {
  return (
    <div className="personShows">
        {credits.map((featured: {backdrop_path: string | null; character?: string; job?: string; id: number; name: string; }, index: Key) => (
            <PersonShow key={index} backdrop_path={featured.backdrop_path} character={featured.character ? featured.character : featured.job} id={featured.id} name={featured.name} />
        ))}
    </div>
  );
}